export const mapping = {
  ...
  "string:url": "urlInput",
  "string:dateTime": "date-picker",
  "string:date": "date-picker",
  "string:year": "date-picker",
  "string:month": "date-picker",
  "string:week": "date-picker",
  "string:quarter": "date-picker",
  "string:time": "time-picker",
  "string:textarea": "textArea",
  "string:color": "color",
  "string:image": "imageInput",
  "range:time": "timeRange",
  "range:dateTime": "date-picker",
  "range:date": "date-picker",
  "range:year": "date-picker",
  "range:month": "date-picker",
  "range:week": "date-picker",
  "range:quarter": "date-picker",
  "*?enum": "radio-group",
  "*?enum_long": "select",
...
export function getWidgetName(schema, _mapping = mapping) {
  ...
  if (props?.options) {
    if (
      (type === "array" && props.options.length > 6) ||
      (type !== "array" && props.options.length > 2)
    ) {
      list.push(`${type}?enum_long`);
      list.push("*?enum_long");
    } else {
  ...
  if (type === "object") {
    list.push(
      (schema.theme === "tile" ? "lineTitle" : schema.theme) || "collapse"
    );
  } else {
...
export const getWidget = (name, widgets) => {
